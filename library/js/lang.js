function LangPack(thisLang){
    let l = {en:{ alg:{}, stat:{}, gn:{}},
             bg:{ alg:{}, stat:{}, gn:{}}
    };
    // general
    l.en.gn.login='Login';
    l.bg.gn.login='Влизане';
    l.en.gn.error='Error';
    l.bg.gn.error='Грешка';
    l.en.gn.selLang='Select a language';
    l.bg.gn.selLang='Изберете език';
    l.en.gn.message='Message';
    l.bg.gn.message='Съобщение';
    l.en.gn.userNameTxt='User name / Fak.№';
    l.bg.gn.userNameTxt='Потребителско име / Фак.№';
    l.en.gn.passTxt='Password';
    l.bg.gn.passTxt='Парола';
    l.en.gn.wrongUserName='Wrong user name / Fak.№';
    l.bg.gn.wrongUserName='Грешно потребителско име / Фак.№';
    l.en.gn.userNotFaund='Username is not found';
    l.bg.gn.userNotFaund='Потребителското име не е намерено';
    l.en.gn.wrongPass='Wrong password';
    l.bg.gn.wrongPass='Грешна парола';

    l.en.gn.ccLFSRLabel='LFSR Encoder';
    l.bg.gn.ccLFSRLabel='LFSR Кодер';
    l.en.gn.infoBits='Information bits';
    l.bg.gn.infoBits='Информационни битове';
    l.en.gn.cwBits='Code Combination';
    l.bg.gn.cwBits='Кодова комбинация';
    l.en.gn.codeParam='Code parameters';
    l.bg.gn.codeParam='Параметри на кода';
    l.en.gn.encoding='encoding';
    l.bg.gn.encoding='кодиране';
    l.en.gn.decoding='decoding';
    l.bg.gn.decoding='декодиране';
    l.en.gn.modeEnc='Mode: Encoding';
    l.bg.gn.modeEnc='Режим: Кодиране';
    l.en.gn.modeDec='Mode: Decoding';
    l.bg.gn.modeDec='Режим: Декодиране';
    l.en.gn.decodedMsg='Decoded message';
    l.bg.gn.decodedMsg='Декодирано съобщение';
    l.en.gn.writeDecMsg='Write decoded message';
    l.bg.gn.writeDecMsg='Запиши декодираното съобщение';
    l.en.gn.cantDecode='The message cannot be decode';
    l.bg.gn.cantDecode='Съобщението не може да бъде декодирано';
    l.en.gn.singleErr='Single error';
    l.bg.gn.singleErr='Единична грешка';
    l.en.gn.doubleErr='Double error';
    l.bg.gn.doubleErr='Двойна грешка';
    l.en.gn.noErr='There is no error';
    l.bg.gn.noErr='Няма грешка';
    l.en.gn.selectErr='Set error status';
    l.bg.gn.selectErr='Задай статус на грешката';
    l.en.gn.errStatus='Error status';
    l.bg.gn.errStatus='Статус на грешката';
    l.en.gn.wrongErrStatus='Wrong error status';
    l.bg.gn.wrongErrStatus='Грешен статус на грешката';
    l.en.gn.wrongDecMsg='Wrong decoded message';
    l.bg.gn.wrongDecMsg='Грешено декодирано съобщение';
    l.en.gn.checkMat='Check the matrix';
    l.bg.gn.checkMat='Провери матрицата';
    l.en.gn.check='Check';
    l.bg.gn.check='Провери';
    l.en.gn.wrongMat='Wrong matrix values';
    l.bg.gn.wrongMat='Грешни стойности на матрицата';
    l.en.gn.setEqu='Set the equation';
    l.bg.gn.setEqu='Задайте уравнението';
    l.en.gn.setVal='Set value';
    l.bg.gn.setVal='Задайте стойност';
    l.en.gn.markRow='Highlight the row';
    l.bg.gn.markRow='Маркирай реда';
    l.en.gn.matrices='Matrices of the Code';
    l.bg.gn.matrices='Матрици на кода';
    l.en.gn.matSize='H matrix size';
    l.bg.gn.matSize='Размерност на H матрицата';
    l.en.gn.matSizeFormat='rows x columns';
    l.bg.gn.matSizeFormat='редове x колони';
    l.en.gn.wrongMatSize='Wrong H matrix size';
    l.bg.gn.wrongMatSize='Грешна размерност на H матрицата';
    l.en.gn.finishMsg='Solving the task is over!';
    l.bg.gn.finishMsg='Решението на задачата приключи!';
    l.en.gn.writeBits='Write the register\'s bits';
    l.bg.gn.writeBits='Напишете битовете на регистъра';

    l.en.gn.models='Model list';
    l.bg.gn.models='Списък на моделите';
    l.en.gn.hcGenModelTitle='Interactive Simulation Model of Hamming Code - General Algorithm';
    l.bg.gn.hcGenModelTitle='Интерактивен Симулационен Модел на Код на Хеминг - Обикновен Алгоритъм';
    l.en.gn.hcMatModelTitle='Interactive Simulation Model of Hamming Code - Matrix Algorithm';
    l.bg.gn.hcMatModelTitle='Интерактивен Симулационен Модел на Код на Хеминг - Матричен Алгоритъм';
    l.en.gn.openMdlBtn='Open the Model';
    l.bg.gn.openMdlBtn='Отвори модела';
    l.en.gn.resetMdlBtn='Reset the Model';
    l.bg.gn.resetMdlBtn='Изчисти модела';
    l.en.gn.encRadio='Encoder';
    l.bg.gn.encRadio='Кодер';
    l.en.gn.decRadio='Decoder';
    l.bg.gn.decRadio='Декодер';
    l.en.gn.mode='Mode:';
    l.bg.gn.mode='Режим:';
    l.en.gn.hammEncGenLabel='Hamming Encoder - General Algorithm';
    l.bg.gn.hammEncGenLabel='Хеминг кодер - обикновен алгоритъм';
    l.en.gn.hammEncMatLabel='Hamming Encoder - Matrix Algorithm';
    l.bg.gn.hammEncMatLabel='Хеминг кодер - матричен алгоритъм';
    l.en.gn.ccEncLabel='Cyclic Code Encoder - Polynomial Algorithm';
    l.bg.gn.ccEncLabel='Кодер на цикличен код - полиномен алгоритъм';
    l.en.gn.ccDecLabel='Cyclic Code Decoder - Polynomial Algorithm';
    l.bg.gn.ccDecLabel='Декодер на цикличен код - полиномен алгоритъм';
    l.en.gn.hammDecMatLabel='Hamming Decoder - Matrix Algorithm';
    l.bg.gn.hammDecMatLabel='Хеминг декодер - матричен алгоритъм';
    l.en.gn.hammDecGenLabel='Hamming Decoder - General Algorithm';
    l.bg.gn.hammDecGenLabel='Хеминг декодер - обикновен алгоритъм';
    l.en.gn.irLabel='Information Register';
    l.bg.gn.irLabel='Информационен регистър';
    l.en.gn.crLabel='Codeword Register';
    l.bg.gn.crLabel='Регистър на кодовата комбинация';
    l.en.gn.wrongK='Wrong "k" value!';
    l.bg.gn.wrongK='Грешна стойност за "k"';
    l.en.gn.wrongN='Wrong "n" value!';
    l.bg.gn.wrongN='Грешна стойност за "n"';
    l.en.gn.setAllBit='Not all information bits are set!';
    l.bg.gn.setAllBit='Не са зададени всички информационни битове!';
    l.en.gn.setAllCwBit='Not all Codeword Bits are Set!';
    l.bg.gn.setAllCwBit='Не са зададени всички битове на кодовата комбинация!';
    l.en.gn.startTimer='Start the timer!';
    l.bg.gn.startTimer='Стартирай таймера!';
    l.en.gn.timerExp='The timer is expired! For new simulation click "Reset the model" button !';
    l.bg.gn.timerExp='Таймерът изтече! За нова симулация  натиснете бутона "Нулиране на модела"!';
    l.en.gn.noModel='The model is not created!';
    l.bg.gn.noModel='Моделът не е създаден!';
    l.en.gn.randHover='Generate Random Bits';
    l.bg.gn.randHover='Генерирай случайни битове';
    l.en.gn.randCwHover='Generate Random Codeword without Error';
    l.bg.gn.randCwHover='Генерирай случайна кодова комбинация без грешка';
    l.en.gn.randErrHover='Generate Random Codeword with Error';
    l.bg.gn.randErrHover='Генерирай случайна кодова комбинация с грешка';
    l.en.gn.placeLabel='Put it in its place';
    l.bg.gn.placeLabel='Постави на мястото му';
    l.en.gn.checkLabels='Check the Labels';
    l.bg.gn.checkLabels='Провери етикетите';
    l.en.gn.incorrectLabel='Incorrect Marked Bits';
    l.bg.gn.incorrectLabel='Грешно маркирани битове';
    l.en.gn.loadBits='Load the Bits';
    l.bg.gn.loadBits='Зареди битовете';
    l.en.gn.loadInfoBits='Load the information bits';
    l.bg.gn.loadInfoBits='Зареди информационните битовете';
    l.en.gn.equBtnTxt='Control Bits Equation';
    l.bg.gn.equBtnTxt='Уравнения на контролените битове';
    l.en.gn.noSelectedBit='There is no Selected Control Bit';
    l.bg.gn.noSelectedBit='Няма избран контролен бит';
    l.en.gn.noEquMem='There is no Selected Equation Members';
    l.bg.gn.noEquMem='Няма избрани членове за уравнението';
    l.en.gn.wrongEqu='Wrong equation';
    l.bg.gn.wrongEqu='Грешно уравнение';
    l.en.gn.wrongCval='Wrong Control Bit Value';
    l.bg.gn.wrongCval='Грешна стойност за контролен бит';
    l.en.gn.wrongCbitCheck='Wrong check value';
    l.bg.gn.wrongCbitCheck='Грешна стойност на проверката';
    l.en.gn.writeCbitVal='Write the Control Bit Value';
    l.bg.gn.writeCbitVal='Запши стойността на контролния бит';
    l.en.gn.noEqu='There is no Equation';
    l.bg.gn.noEqu='Няма уравнение';
    l.en.gn.calcCbit='Calculate the equation';
    l.bg.gn.calcCbit='Изчисли уравнението';
    l.en.gn.insertToEqu='Insert to the Equation';
    l.bg.gn.insertToEqu='Добави към уравнението';
    l.en.gn.selectCbit='Select the Control Bit';
    l.bg.gn.selectCbit='Избери контролния бит';
    l.en.gn.anotherSelCbit='Another control bit is selected';
    l.bg.gn.anotherSelCbit='Има избран друг контролен бит';
    l.en.gn.wronC0choice='Wrong Control Bit Choice';
    l.bg.gn.wronC0choice='Грешен избор на контролен бит';
    l.en.gn.wasCalculated='This Control Bit is calculated';
    l.bg.gn.wasCalculated='Този контролен бит е изчислен';
    l.en.gn.wasChecked='This control bit already was checked';
    l.bg.gn.wasChecked='Този контролен бит е проверен вече';
    l.en.gn.writeCbitCheck='Write Control bit value';
    l.bg.gn.writeCbitCheck='Запиши стойността на контролния бит';
    l.en.gn.writeCbitCheck='Write the check value';
    l.bg.gn.writeCbitCheck='Запиши стойността на проверката';
    l.en.gn.checkRes='CB';
    l.bg.gn.checkRes='КБ';
    l.en.gn.randCWLabel='Random CW';
    l.bg.gn.randCWLabel='Случайна КД';
    l.en.gn.randCwErrLabel='Random CW with Error';
    l.bg.gn.randCwErrLabel='Случайна КД с грешка';
    l.en.gn.randBitsLabel='Random Bits';
    l.bg.gn.randBitsLabel='Случайни битове';
    l.en.gn.setDecCode='Set the Error Position';
    l.bg.gn.setDecCode='Задайте позицията на грешката';
    l.en.gn.errorPos='Error position';
    l.bg.gn.errorPos='Позиция на грешката';
    l.en.gn.setBinCode='Set the Error Binary Code';
    l.bg.gn.setBinCode='Задай двоичния код на грешката';
    l.en.gn.wrongBinCode='Wrong the Error Binary Code';
    l.bg.gn.wrongBinCode='Грешен двоичен код на грешката';
    l.en.gn.wrongDecCode='Wrong error position';
    l.bg.gn.wrongDecCode='Грешна позиция на грешката';
    l.en.gn.resultAnalysis='Result Analysis';
    l.bg.gn.resultAnalysis='Анализ на резултата';
    l.en.gn.errSyndrome='Error syndrome';
    l.bg.gn.errSyndrome='Синдром на грешката';
    l.en.gn.wrongSyndrome='Wrong syndrome';
    l.bg.gn.wrongSyndrome='Грешен синдром на грешката';
    l.en.gn.mTitle='Number of information bits';
    l.bg.gn.mTitle='Брой на информационните битове';
    l.en.gn.lTitle='Number of errors the code can detect';
    l.bg.gn.lTitle='Брой на грешките, които кода може да открива';
    l.en.gn.kTitle='Number of control bits';
    l.bg.gn.kTitle='Брой на контролните битове';
    l.en.gn.nTitle='Number of codeword bits';
    l.bg.gn.nTitle='Брой на битовете на кодовата комбинация';
    l.en.gn.regBitHover='Set value';
    l.bg.gn.regBitHover='Задай стойност';
    l.en.gn.showHmat='Show the H matrix';
    l.bg.gn.showHmat='Покажи H матрицата';
    l.en.gn.H_matColsLabel='H\' matrix columns';
    l.bg.gn.H_matColsLabel='Колоните на H\'';
    l.en.gn.H_matColsList='Set columns order of H\'';
    l.bg.gn.H_matColsList='Задайте подредбата на колоните на H\'';
    l.en.gn.wrongCol='Wrong column order of the H\' matrix';
    l.bg.gn.wrongCol='Грешна подреба на колоните на H\' матрицата';
    l.en.gn.wrongLabel='Wrong label';
    l.bg.gn.wrongLabel='Грешен етикет';
    l.en.gn.wrongLabelHmat='Wrong labels of the H matrix';
    l.bg.gn.wrongLabelHmat='Грешни етикети на H матрицата';
    l.en.gn.wrongLabelH_mat='Wrong H\' matrix column labels';
    l.bg.gn.wrongLabelH_mat='Грешни етикети на колоните на H\' матрицата';
    l.en.gn.wrongLabelHmat='Wrong H matrix column labels';
    l.bg.gn.wrongLabelHmat='Грешни етикети на колоните на H матрицата';
    l.en.gn.dblclick='Double-click for editing';
    l.bg.gn.dblclick='Двойно кликване за редактиране';
    l.en.gn.cyclicPolyModelTitle='Interactive Simulation Model of Cyclic Code - Polynomial Algorithm';
    l.bg.gn.cyclicPolyModelTitle='Интерактивен Симулационен Модел на цикличен Код - Полинимен Алгоритъм';
    l.en.gn.cyclicLFSRModelTitle='Interactive Simulation Model of Cyclic Code - LFSR';
    l.bg.gn.cyclicLFSRModelTitle='Интерактивен Симулационен Модел на цикличен Код - LFSR';
    l.en.gn.genPoly='Generator polynomial';
    l.bg.gn.genPoly='Генераторен полином';
    l.en.gn.selGenPoly='Select polynomial';
    l.bg.gn.selGenPoly='Изберете полином';
    l.en.gn.genPolys='Generator polynomials';
    l.bg.gn.genPolys='Генераторени полиноми';
    l.en.gn.fbLabel='FB = ';
    l.bg.gn.fbLabel='ОВ = ';
    l.en.gn.wrongGenPoly='Wrong generator polynomial';
    l.bg.gn.wrongGenPoly='Грешен генераторен полином';
    l.en.gn.wrongMul='Wrong polynomial multiplicator';
    l.bg.gn.wrongMul='Грешен полиномен множител';
    l.en.gn.wrongRes='Wrong result';
    l.bg.gn.wrongRes='Грешен резултат';
    l.en.gn.wrongRem='Wrong remainder';
    l.bg.gn.wrongRem='Грешен остатък';
    l.en.gn.wrongXor='Wrong XOR value!';
    l.bg.gn.wrongXor='Грешнa XOR стойност!';
    l.en.gn.wrongSw='Wrong switch position!';
    l.bg.gn.wrongSw='Грешна позиция на ключовете!';
    l.en.gn.wrongRemBin='Wrong binary remainder!';
    l.bg.gn.wrongRemBin='Грешен двоичен остатък!';
    l.en.gn.wrongFb='Wrong Feedback value!';
    l.bg.gn.wrongFb='Грешна стойност на ОВ!';
    l.en.gn.shiftHover='Shift the register';
    l.bg.gn.shiftHover='Измести регистъра';
    l.en.gn.flipHover='Fliping the register';
    l.bg.gn.flipHover='Обръщане на регистъра';
    l.en.gn.flipBtnLabel='Flip';
    l.bg.gn.flipBtnLabel='Обръщане';
    l.en.gn.swHover='Change position';
    l.bg.gn.swHover='Промени позицията';
    l.en.gn.xorHover='Calculate XOR';
    l.bg.gn.xorHover='Изчисли XOR';
    l.en.gn.fbHover='Calculate FB';
    l.bg.gn.fbHover='Изчисли ОВ';
    l.en.gn.newCycle='New cycle';
    l.bg.gn.newCycle='Нов цикъл';
    l.en.gn.lastCycle='Last cycle';
    l.bg.gn.lastCycle='Последен цикъл';
    l.en.gn.notComplCycle='Not completed cycle';
    l.bg.gn.notComplCycle='Не завършен цикъл';
    l.en.gn.wrongOper='Wrong operation';
    l.bg.gn.wrongOper='Грешна операция';
    l.en.gn.solveTime='Time to solve';
    l.bg.gn.solveTime='Време за решаване';

    l.en.gn.infoBitPoly='Polynomial of the Information bits';
    l.bg.gn.infoBitPoly='Полином на информационните битове';
    l.en.gn.polyDivision='Polynomial division';
    l.bg.gn.polyDivision='Деление на полиномите';
    l.en.gn.cwFormula='Codeword polynomial formula';
    l.bg.gn.cwFormula='Фромула на кодовата комбинация';
    l.en.gn.polyDivisible='Polynomial divisible';
    l.bg.gn.polyDivisible='Полином на делимото';
    l.en.gn.wrongInfoPoly='Wrong information bit polynomial';
    l.bg.gn.wrongInfoPoly='Грешен полином на информационните битове';
    l.en.gn.wrongCWformula='Wrong codeword formula';
    l.bg.gn.wrongCWformula='Грешна формула на кодовата комбинация';
    l.en.gn.wrongXk_InfoPoly='Wrong result of the multiplication of x^k.G(x)';
    l.bg.gn.wrongXk_InfoPoly='Грешен резултат от умножението x^k.G(x)';
    l.en.gn.wrongCwPoly='Wrong codeword polynomial';
    l.bg.gn.wrongCwPoly='Грешен полином на кодовата комбинация';
    l.en.gn.wrongCwBin='Wrong binary codeword';
    l.bg.gn.wrongCwBin='Грешна двоична кодова комбинация';
    l.en.gn.remBin='The remainder in binary form';
    l.bg.gn.remBin='Остатъкът в двоична формат';
    l.en.gn.cwPoly='Codeword in polynomial form';
    l.bg.gn.cwPoly='Кодовата комбинация в полиномна форма';
    l.en.gn.cwBin='Codeword in binary form';
    l.bg.gn.cwBin='Кодовата комбинация в двоична форма';

    // algorithm
    l.en.alg.modelControl='Model Control';
    l.bg.alg.modelControl='Управление';
    l.en.alg.runStep='Run the Step';
    l.bg.alg.runStep='Изпълни стъпката';
    l.en.alg.autoRun='Auto Solve';
    l.bg.alg.autoRun='Автоматично решение';
    l.en.alg.stopAutoRun='Stop auto solve';
    l.bg.alg.stopAutoRun='Спри автоматичното решение';
    l.en.alg.speedRun='Speed for auto solve';
    l.bg.alg.speedRun='Скорост за автоматично решение';
    l.en.alg.speed=['Very slow', 'Slow', 'Medium','Fast','Very fast'];
    l.bg.alg.speed=['Мн. бавна', 'Бавна', 'Средна','Бърза','Мн. бърза'];
    l.en.alg.infoPoly='Information bit in polynomial form';
    l.bg.alg.infoPoly='Информационните битове в полиномна форма';
    l.en.alg.infoPolyHelp='Write the polynomial of information bits';
    l.bg.alg.infoPolyHelp='Напишете полинома на информационните битове';
    l.en.alg.xk_InfoPoly='Increasing the information polynomial degree';
    l.bg.alg.xk_InfoPoly='Увеличаване степента на информационния полином';
    l.en.alg.xk_InfoPolyHelp='Multiply the information polynomial by x^k';
    l.bg.alg.xk_InfoPolyHelp='Умножете информационния полином с x^k';
    l.en.alg.cwFormula='Codeword formula';
    l.bg.alg.cwFormula='Формула на кодовата комбинация';
    l.en.alg.cwFormulaHelp='Write the codeword formula';
    l.bg.alg.cwFormulaHelp='Напишете формулата на кодовата комбинация';
    l.en.alg.writeMul='Polynomial quotient determination';
    l.bg.alg.writeMul='Определяне члена на полиномното частно';
    l.en.alg.writeMulHelp='Click on "'+l.en.gn.newCycle+'" button after write the next member of polynomial quotient';
    l.bg.alg.writeMulHelp='Натиснете върху бутона "'+l.bg.gn.newCycle+'" след това напишете поредния член на полиномното частно';
    l.en.alg.writeRes='Multiplying the quotient by the divisor';
    l.bg.alg.writeRes='Умножение на члена на частното с делителя';
    l.en.alg.writeResHelp='Write the result of multiplying the next member of coefficient by the divisor';
    l.bg.alg.writeResHelp='Напишете резултата от умножението на поредния член на частното с делителя'
    l.en.alg.writeRem='Calculating the remainder';
    l.bg.alg.writeRem='Изчисляване на остатъка';
    l.en.alg.writeRemHelp='Sum the polynomial result of the multiplication and the remainder';
    l.bg.alg.writeRemHelp='Сумирайте полинома на получения резултат от умножението и остатъка'
    l.en.alg.remBin='Remainder in binary form';
    l.bg.alg.remBin='Остатък в двоична форма';
    l.en.alg.remBinHelp='Write the remainder of the division in binary form';
    l.bg.alg.remBinHelp='Запишете остатъка в двоична форма';
    l.en.alg.cwPoly='The codeword in binary form';
    l.bg.alg.cwPoly='Кодовата комбинация в полиномна форма';
    l.en.alg.cwPolyHelp='Write the codeword in polynomial form';
    l.bg.alg.cwPolyHelp='Запишете кодовата комбинация в двоична форма'
    l.en.alg.cwBin='The codeword in binary form';
    l.bg.alg.cwBin='Кодовата комбинация в двоична форма';
    l.en.alg.cwBinHelp='Write the codeword in binary form';
    l.bg.alg.cwBinHelp='Запишете кодовата комбинация в двоична форма'
    l.en.alg.divExitCond='R(x)  <  P(x)';
    l.bg.alg.divExitCond='R(x)  <  P(x)';

    l.en.alg.mTimesExitCond='Last info bit ?';
    l.bg.alg.mTimesExitCond='Последен инф. бит ?';
    l.en.alg.kTimesExitCond='Last control bit ?';
    l.bg.alg.kTimesExitCond='Последен контролен ?';

    l.en.alg.setpolyDivisible='Polynomial divisible formation';
    l.bg.alg.setpolyDivisible='Образуване на полиномното делимо';
    l.en.alg.setpolyDivisibleHelp='';
    l.en.alg.setpolyDivisibleHelp='';
    l.bg.alg.setpolyDivisibleHelp='';

    l.en.alg.finish='Solving the task is over!';
    l.bg.alg.finish='Решението на задачата приключи!';
    l.en.alg.showEndMsg='Show the end message';
    l.bg.alg.showEndMsg='Покажи съобщението за край';
    l.en.alg.setBits='Set the Information Bits';
    l.bg.alg.setBits='Задаване на инф. битове';
    l.en.alg.setBitsCW='Set the Codeword Bits';
    l.bg.alg.setBitsCW='Задаване на кодовата комбинация';
    l.en.alg.setHmat='Set the H matrix';
    l.bg.alg.setHmat='Задаване на H матрицата';
    l.en.alg.setHmatSize='Set the H matrix size';
    l.bg.alg.setHmatSize='Задаване на размерността на H матрицата';
    l.en.alg.setH_mat='Set the H\' matrix';
    l.bg.alg.setH_mat='Задаване на H\' матрицата';
    l.en.alg.markBitsDEC='Marking the Decoder Bits';
    l.bg.alg.markBitsDEC='Маркиране битовете на декодера';
    l.en.alg.markBitsEN='Marking the Encoder Bits';
    l.bg.alg.markBitsEN='Маркиране битовете на кодера';
    l.en.alg.loadEN='Loading the Encoder';
    l.bg.alg.loadEN='Зареждане на кодера';
    l.en.alg.loadDEC='Loading the Decoder';
    l.bg.alg.loadDEC='Зареждане на декодера';
    l.en.alg.calcCbits='Calculating the Control Bit';
    l.bg.alg.calcCbits='Изчисляване на контролния бит';
    l.en.alg.calcEqu='Calculating the Equation';
    l.bg.alg.calcEqu='Изчисляване на уравнението';
    l.en.alg.selectCbit='Selecting а Control Bit';
    l.bg.alg.selectCbit='Избиране на контролен бит';
    l.en.alg.createEqu='Creating an Equation';
    l.bg.alg.createEqu='Съставяне на уравнение';
    l.en.alg.lastCbit='Last control bit?';
    l.bg.alg.lastCbit='Последен контролен бит?';
    l.en.alg.calcCbit='Calculating the Control Bit';
    l.bg.alg.calcCbit='Изчисляване на контролен бит';
    l.en.alg.checkCbit='Check the Control Bit';
    l.bg.alg.checkCbit='Проверка на контролен бит';
    l.en.alg.writeCbit='Writing the Control Bit';
    l.bg.alg.writeCbit='Записване на контролния бит';
    l.en.alg.yes='Yes';
    l.bg.alg.yes='Да';
    l.en.alg.no='No';
    l.bg.alg.no='Не';
    l.en.alg.title='Algorithm';
    l.bg.alg.title='Алгоритъм';
    l.en.alg.wrongOper='Wrong operation';
    l.bg.alg.wrongOper='Грешна операция';
    l.en.alg.setParam='Setting the Code Parameters';
    l.bg.alg.setParam='Задаване на параметрите на кода';
    l.en.alg.resAnalysis='Analysis of the Result';
    l.bg.alg.resAnalysis='Анализ на резултата';
    l.en.alg.flipIR='Revers the Information Register.';
    l.bg.alg.flipIR='Обръщане на инф. регистър';
    l.en.alg.setSW='Set the Switches';
    l.bg.alg.setSW='Превключване на ключовете';
    l.en.alg.calcFB='Calculate the Feedback value';
    l.bg.alg.calcFB='Изчисляване стойн. на ОВ';
    l.en.alg.calcXOR='Calculate the XORs';
    l.bg.alg.calcXOR='Изчисляване стойн. на XOR';
    l.en.alg.calcXOR='Calculate the XORs';
    l.bg.alg.calcXOR='Изчисляване стойн. на XOR';
    l.en.alg.shiftEN='Shift the Encoder';
    l.bg.alg.shiftEN='Изместване на Кодера';
    l.en.alg.shiftCR='Shift the Codeword Register';
    l.bg.alg.shiftCR='Изместване рег. на кодовата комбинация';
    l.en.alg.shiftIR='Shift the Information Register';
    l.bg.alg.shiftIR='Изместване рег. на Информ. битове';
    l.en.alg.shiftParity='Shift Parity Bits from Encoder to Codeword Register';
    l.bg.alg.shiftParity='Изместване на контр. битове в рег. на КД';
    l.en.alg.flipCR='Reverse the Codeword Register';
    l.bg.alg.flipCR='Обръщане реги. на кодовата комбинация';
    l.en.alg.emptyIR='Is IR empty ?';
    l.bg.alg.emptyIR='Празен ли е ИР ?';
    l.en.alg.emptyEN='Is Encoder empty ?';
    l.bg.alg.emptyEN='Празен ли е кодера ?';

    l.en.alg.setParamHelp='Set the value of: m, l, k and n';
    l.bg.alg.setParamHelp='Задайте стойностите на: m, l, k и n';
    l.en.alg.setIrBitsHelp='Click on Info register\'s bits or "'+l.en.gn.randBitsLabel+'"';
    l.bg.alg.setIrBitsHelp='Кликнете върху битовете на Информационния регисър или бърху "'+l.bg.gn.randBitsLabel+'"';
    l.en.alg.markBitsENHelp='Drag all labels on the correct bits after click the "'+l.en.gn.checkLabels+'" button';
    l.bg.alg.markBitsENHelp='Провлачете всички етикети върху правилните битове и след това кликнете върху бутона "'+l.bg.gn.checkLabels+'"';
    l.en.alg.setHmatHelp='Set all values of the H matrix by clicking on the elements of the matrix';
    l.bg.alg.setHmatHelp='Задайте стойностите на H матрицата чрез кликане върху елементите на матрицата ';
    l.en.alg.setHmatSizeHelp='Write the H matrix size as: rows x columns';
    l.bg.alg.setHmatSizeHelp='Запишете размерността на H матрицата като: редове x колони';
    l.en.alg.setH_matHelp='Set the H\' matrix column labels after set the matrix values if required';
    l.bg.alg.setH_matHelp='Задайте етикети на матрицата H\', след това задайте стойностите на матрицата ако има C0';
    l.en.alg.loadBitsHelp='Click on "'+l.en.gn.loadBits+'" button';
    l.bg.alg.loadBitsHelp='Кликнете върху бутона "'+l.bg.gn.loadBits+'"';
    l.en.alg.loadInfoBitsHelp='Click on "'+l.en.gn.loadInfoBits+'" button';
    l.bg.alg.loadInfoBitsHelp='Кликнете върху бутона "'+l.bg.gn.loadInfoBits+'"';
    l.en.alg.selectCbitHelp='Click on the control bit that we will calculate / check';
    l.bg.alg.selectCbitHelp='Кликнете върху контролния бит, който ще изчислявате / проверявате';
    l.en.alg.createEquHelp='Write the equation of selected control bit by double click on equation';
    l.bg.alg.createEquHelp='Напишете уравнението на избрания контролен бит чрез двойно кликане върху уравнеието';
    l.en.alg.createEquHelp1='Click on the bits to add them to the equation';
    l.bg.alg.createEquHelp1='Кликнете върху битовете участващи в уравнението';
    l.en.alg.calcCbitHelp='Click on the binary part of equation';
    l.bg.alg.calcCbitHelp='Кликнете върху двоичната част на уравнението';
    l.en.alg.calcEquHelp='Click on the binary part of equation';
    l.bg.alg.calcEquHelp='Кликнете върху двоичната част на уравнението';

    l.en.alg.checkCbitHelp='Click on the binary part of equation';
    l.bg.alg.checkCbitHelp='Кликнете върху двоичната част на уравнението';
    l.en.alg.writeCbitHelp='Click the selected control bit';
    l.bg.alg.writeCbitHelp='Кликнете върху избрания контролен бит';
    l.en.alg.setCwBitsHelp='Click on CW register\'s bits or "'+l.en.gn.randCWLabel+'" or "'+l.en.gn.randCwErrLabel+'"';
    l.bg.alg.setCwBitsHelp='Кликнете върху битовете на Регистъра на кодовата комбинация или върху "'+l.bg.gn.randCWLabel+'" или върху "'+l.bg.gn.randCwErrLabel+'"';
    l.en.alg.writeCbitCheckHelp='Click on the checked control bit, below the equation';
    l.bg.alg.writeCbitCheckHelp='Кликнете върху проверявания контролен бит, под уравнението';
    l.en.alg.errAnalysisHelp='Define the syndrome of error. Determine the status of the error. Write the position of the error, if possible.';
    l.bg.alg.errAnalysisHelp='Определете синдрома (коректора) на грешката. Определете статуса на грешката. Запишете позицията на грешката, ако е възможно.';

    l.en.alg.reverseIRHelp='Click the (R) button on the information register';
    l.bg.alg.reverseIRHelp='Кликнете върху бутона (R) на информационния регистър';
    l.en.alg.setSWHelp='Click on the switches';
    l.bg.alg.setSWHelp='Кликнете върху ключовете';
    l.en.alg.calcFBHelp='Click the last XOR item.';
    l.bg.alg.calcFBHelp='Кликнете върху последния XOR елемент';
    l.en.alg.calcXORHelp='Click on each XOR element without the last one.';
    l.bg.alg.calcXORHelp='Кликнете върху всичките XOR елементи, без последния';
    l.en.alg.shiftENHelp='Click the (>>>) button on the Encoder.';
    l.bg.alg.shiftENHelp='Кликнете върху бутона (>>>) на кодера';
    l.en.alg.shiftCRHelp='Click the (>>>) button on the Codeword register.';
    l.bg.alg.shiftCRHelp='Кликнете върху бутона (>>>) на регистъра на кодовата комбинация';
    l.en.alg.shiftIRHelp='Click the (>>>) button on the Infor register.';
    l.bg.alg.shiftIRHelp='Кликнете върху бутона (>>>) на информационния регистър';
    l.en.alg.flipIRHelp='Click the ('+l.en.gn.flipBtnLabel+') button on the Info register.';
    l.bg.alg.flipIRHelp='Кликнете върху бутона ('+l.bg.gn.flipBtnLabel+') на информационния регистър';
    l.en.alg.flipCRHelp='Click the ('+l.en.gn.flipBtnLabel+') button on the Codeword register.';
    l.bg.alg.flipCRHelp='Кликнете върху бутона ('+l.bg.gn.flipBtnLabel+') на регистъра на кодовата комбинация';

    // statistics
    l.en.stat.title='Statistics';
    l.bg.stat.title='Статистика';
    l.en.stat.startTimer='Start the Timer';
    l.bg.stat.startTimer='Стартирай таймера';
    l.en.stat.stopTimer='Stop the Timer';
    l.bg.stat.stopTimer='Спри таймера';
    l.en.stat.time='Time:';
    l.bg.stat.time='Време:';
    l.en.stat.min='min';
    l.bg.stat.min='мин';
    l.en.stat.sec='sec';
    l.bg.stat.sec='сек';
    l.en.stat.maxTime='The maximum time is: ';
    l.bg.stat.maxTime='Максималното време е: ';
    l.en.stat.errHistory='Errors History';
    l.bg.stat.errHistory='История на грешките';
    l.en.stat.err='Errors: ';
    l.bg.stat.err='Грешки: ';
    l.en.stat.showErrors='Show Errors';
    l.bg.stat.showErrors='Покажи грешките';
    l.en.stat.hideErrors='Hide Errors';
    l.bg.stat.hideErrors='Скрий грешките';

    return (thisLang === 'bg') ? l.bg : l.en
}